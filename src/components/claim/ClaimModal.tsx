...
import { CloseIcon, CustomLightSpinner, ExternalLink, TYPE, TokenAnimated } from '../../theme'
      ...
      {!attempting && !claimConfirmed && (
        <ContentWrapper gap="lg">
          <ModalUpper>
            ...
            <CardSection gap="md">
              <RowBetween>
                <TYPE.white fontWeight={500}>Claim YAPE</TYPE.white>
                <CloseIcon onClick={toggleClaimModal} style={{ zIndex: 99 }} color="white" />
              </RowBetween>
              <TYPE.white fontWeight={700} fontSize={36}>
                {unclaimedAmount?.toFixed(0, { groupSeparator: ',' } ?? '-')} YAPE
              </TYPE.white>
            </CardSection>
            <Break />
            <CardSection gap="sm">
              {userClaimData?.flags?.isSOCKS && (
                <RowBetween>
                  ...
                  <TYPE.subHeader color="white">{SOCKS_AMOUNT} YAPE</TYPE.subHeader>
                </RowBetween>
              )}
              ...
              {userClaimData?.flags?.isUser && (
                <RowBetween>
                  ...
                  <TYPE.subHeader color="white">{USER_AMOUNT} YAPE</TYPE.subHeader>
                </RowBetween>
              )}
            </CardSection>
          </ModalUpper>
          <AutoColumn gap="md" style={{ padding: '1rem', paddingTop: '0' }} justify="center">
            <TYPE.subHeader fontWeight={500}>
              As a member of the Uniswap community you may claim YAPE to be used for voting and governance. <br />{' '}
              <br />
              <ExternalLink href="https://uniswap.org/blog/uni">Read more about YAPE</ExternalLink>
            </TYPE.subHeader>
            <ButtonPrimary
        ...
        <ConfirmOrLoadingWrapper activeBG={true}>
          ...
          <ConfirmedIcon>
            {!claimConfirmed ? (
              ...
              <TokenAnimated width="72px" src={tokenLogo} />
            )}
          </ConfirmedIcon>
          <AutoColumn gap="100px" justify={'center'}>
            <AutoColumn gap="12px" justify={'center'}>
              ...
              {!claimConfirmed && (
                <Text fontSize={36} color={'#f94144'} fontWeight={800}>
                  {unclaimedAmount?.toFixed(0, { groupSeparator: ',' } ?? '-')} YAPE
                </Text>
              )}
            </AutoColumn>
